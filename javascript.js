...
/* ================================================================
   MEJORAS VISUALES — Animaciones avanzadas + WA flotante
================================================================ */

/* ── Variantes de animación de entrada ── */
(function initSRVariants() {
    // Añadir clases a elementos específicos para animaciones direccionales
    document.querySelectorAll('.info-card:nth-child(odd)').forEach(el => {
        if (!el.classList.contains('sr-target')) return;
        el.classList.add('sr-left');
    });
    document.querySelectorAll('.info-card:nth-child(even)').forEach(el => {
        if (!el.classList.contains('sr-target')) return;
        el.classList.add('sr-right');
    });

    // Observer para las variantes direccionales
    const obs = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (!entry.isIntersecting) return;
            const el = entry.target;
            const delay = parseFloat(getComputedStyle(el).getPropertyValue('--delay')) || 0;
            setTimeout(() => el.classList.add('sr-visible'), delay * 1000);
            obs.unobserve(el);
        });
    }, { threshold: 0.12 });

    document.querySelectorAll('.sr-left, .sr-right, .sr-scale').forEach(el => {
        obs.observe(el);
    });
})();

/* ── WhatsApp flotante: número dinámico desde textos ── */
(function initFloatWA() {
    const floatBtn = document.querySelector('.float-wa');
    if (!floatBtn) return;

    // Al cargar textos dinámicos, actualizar el href del botón flotante
    const observer = new MutationObserver(() => {
        const waLinks = document.querySelectorAll('a[href*="wa.me"]');
        waLinks.forEach(a => {
            if (a !== floatBtn && a.href.includes('wa.me')) {
                floatBtn.href = a.href;
            }
        });
    });
    observer.observe(document.body, { subtree: true, attributes: true, attributeFilter: ['href'] });

    // Mostrar con animación de entrada después de 2s
    floatBtn.style.opacity = '0';
    floatBtn.style.transform = 'scale(0) translateY(20px)';
    floatBtn.style.transition = 'opacity .5s ease, transform .5s cubic-bezier(.34,1.56,.64,1)';
    setTimeout(() => {
...
(function initParallax() {
    ...
    window.addEventListener('scroll', () => {
        ...
        if (scroll < window.innerHeight) {
            heroBg.style.backgroundPositionY = `calc(50% + ${scroll * 0.3}px)`;
        }
    }, { passive: true });
})();

/* ── Hover 3D suave en tarjetas de servicios ── */
